<#
.SYNOPSIS
    Pester tests for Office365MailAliases module

.DESCRIPTION
    Comprehensive Pester tests covering all public functions in the Office365MailAliases module.
    These tests use mocking to avoid requiring actual Exchange Online connections.

.NOTES
    Author: Office365MailAliases Contributors
#>

BeforeAll {
    # Create a stub module for ExchangeOnlineManagement to satisfy #requires statements
...
'@
    ...
    # Import the stub module first to satisfy requirements
    Import-Module $stubModulePath -Force -Global
    
    # Import the module under test
    $ModulePath = Join-Path $PSScriptRoot '..' 'Office365MailAliases' 'Office365MailAliases.psm1'
    ...
    Mock Add-DistributionGroupMember { } -ModuleName Office365MailAliases
...
Describe 'New-MailAlias' {
    ...
    Context 'Typical Usage' {
        ...
        It 'Should create specified number of aliases' {
            ...
            Should -Invoke New-DistributionGroup -ModuleName Office365MailAliases -Exactly 3
        }
        ...
        It 'Should connect to Exchange Online when no session exists' {
            ...
            Should -Invoke Connect-ExchangeOnline -ModuleName Office365MailAliases -Exactly 1
        }
        ...
        It 'Should disconnect from Exchange Online when KeepAlive is not specified' {
            ...
            Should -Invoke Disconnect-ExchangeOnline -ModuleName Office365MailAliases -Exactly 1
        }
        ...
        It 'Should not disconnect from Exchange Online when KeepAlive is specified' {
            ...
            Should -Invoke Disconnect-ExchangeOnline -ModuleName Office365MailAliases -Exactly 0
        }

        It 'Should set distribution group properties correctly' {
            New-MailAlias -NumberOfAliases 1 -EmailDomain 'contoso.com' -Owner 'admin@contoso.com' -GroupNamePrefix 'TEST'

            Should -Invoke Set-DistributionGroup -ModuleName Office365MailAliases -Exactly 1
            Should -Invoke Add-RecipientPermission -ModuleName Office365MailAliases -Exactly 1
            Should -Invoke Add-DistributionGroupMember -ModuleName Office365MailAliases -Exactly 1
        }
    }
    ...
    Context 'Edge Cases' {
        It 'Should skip creation when distribution group name already exists' {
            ...
            Should -Invoke New-DistributionGroup -ModuleName Office365MailAliases -Exactly 0
        }
...
Describe 'Select-MailAlias' {
    ...
    Context 'Typical Usage - Existing Alias' {
        ...
        It 'Should not modify existing alias' {
            ...
            Should -Invoke Set-DistributionGroup -ModuleName Office365MailAliases -Exactly 0
        }
    }
    ...
    Context 'Edge Cases' {
        ...
        It 'Should warn when alias is less than 60 minutes old' {
            ...
            Mock Set-DistributionGroup { } -ModuleName Office365MailAliases
            Mock Write-Warning { } -ModuleName Office365MailAliases

            Select-MailAlias -DomainName 'NewDomain.com' -WarningAction SilentlyContinue

            Should -Invoke Write-Warning -ModuleName Office365MailAliases -Exactly 1
        }
    }
}
...
Describe 'Set-MailAliasToArchived' {
    ...
    Context 'Typical Usage' {
        ...
        It 'Should remove members from distribution group' {
            ...
            Should -Invoke Remove-DistributionGroupMember -ModuleName Office365MailAliases -Exactly 1
        }
